...
!       of this field means that the owning TAVP-MNG is not (yet) known. Sometimes, when
!       a tensor does not split at that level of hierarchy, it will have two separate
!       TAVP-MNG owner ids, one as a parent tensor and one as a child tensor, both
!       instances being the same tensor. Consequently, its owner id will depend on
!       the role it is expected to have during the request (parent or child).
...
!TYPES:
 !Tensor argument cache entry (TAVP-specific):
        type, extends(tens_cache_entry_t), private:: tens_entry_mng_t
         ...
         contains
          ...
          procedure, public:: has_one_owner=>TensEntryMngHasOneOwner  !returns TRUE if the owner id is the same as a parent and as a child, FALSE otherwise
 ...
 !TAVP-MNG decoder:
        type, extends(ds_decoder_t), private:: tavp_mng_decoder_t
         ...
         integer(INTD), private:: decodes_from                      !direction in the TAVP hierarchy the decoder will be decoding from
 ...
 !TAVP-MNG decoder configuration:
        type, extends(dsv_conf_t), private:: tavp_mng_decoder_conf_t
         ...
         integer(INTD), public:: decodes_from                       !direction in the TAVP hierarchy the decoder will be decoding from
 ...
 !TAVP-MNG (order of DSVU matters):
...
!VISIBILITY:
 ...
 !tens_entry_mng_t:
        ...
        private TensEntryMngHasOneOwner
...
!Sets the tensor owner id (if needed, id as a child will be different).
         ...
         integer(INTD), intent(out), optional:: ierr    !out: error code
         integer(INTD), intent(in), optional:: as_child !in: if present, this will be the id as a child (may differ from <id> above)
         integer(INTD):: errc
...
!--------------------------------------------------------------
        function TensEntryMngHasOneOwner(this,ierr) result(res)
!Returns TRUE if the tensor cache entry has the same owner as a parent and as a child,
!FALSE otherwise. In case the tensor cache entry does not have any owner, TRUE is returned.
         implicit none
         ...
         errc=0
         call this%lock()
         res=(this%owner_id(1).eq.this%owner_id(2))
         call this%unlock()
         if(present(ierr)) ierr=errc
         return
        end function TensEntryMngHasOneOwner
!--------------------------------------------------------------
           ...
           if(tensor%is_set(errc)) then
            if(errc.eq.TEREC_SUCCESS) then
             ...
             if(present(owner)) then
              this%owner_id(:)=owner(:) !metadata owner (TAVP-MNG id), as a parent and as a child (may differ in general)
             else
              this%owner_id(:)=-1
             endif
            else
           ...
           if(tens_cache_entry%is_set(errc)) then
            if(errc.eq.0) then
             ...
             if(errc.eq.0) then
              ...
              this%owner_id(1:2)=(/pid,cid/) !import owner ids from the tensor cache entry
         ...
         if(this%is_active(errc)) then
          if(errc.eq.0) then
           ...
           if(associated(cache_entry)) then
            ...
            this%cache_entry=>NULL()
           !this%owner_id(:)=-1 !`owner_id is kept unchanged
           endif
          else
           errc=-2
          endif
...
!------------------------------------------------------------------
        function TensOprndGetOwnerId(this,ierr,as_child) result(id)
!Returns the tensor owner id, either as a parent or as a child (if differ).
         implicit none
         integer(INTD):: id                          !out: tensor owner id
         class(tens_oprnd_t), intent(inout):: this   !in: active tensor operand
         integer(INTD), intent(out), optional:: ierr !out: error code
         logical, intent(in), optional:: as_child    !in: if TRUE, as-a-child owner id will be returned (defaults to FALSE)
         integer(INTD):: errc
...
!----------------------------------------------------------------------------
        subroutine TensOprndSetOwnerId(this,owner,ierr,update_cache,as_child)
!Sets the tensor owner id, either as a parent or as a child, with or without tensor cache update.
         implicit none
         class(tens_oprnd_t), intent(inout):: this      !inout: active tensor operand
         integer(INTD), intent(in):: owner              !in: tensor owner id (no restrictions)
         integer(INTD), intent(out), optional:: ierr    !out: error code
         logical, intent(in), optional:: update_cache   !in: if TRUE, the owner id in the tensor cache will be updated as well (defaults to FALSE)
         integer(INTD), intent(in), optional:: as_child !in: if provided, this is the owner id as a child (may be different)
         integer(INTD):: errc
          ...
          if(errc.eq.0) then
           ...
           if(present(update_cache)) then
            if(update_cache) then
             if(associated(this%cache_entry)) then
              call this%cache_entry%set_owner_id(this%owner_id(1),errc,as_child=this%owner_id(2)); if(errc.ne.0) errc=-4
             else
              errc=-3
             endif
            endif
...
!If there is no associated cache entry, does nothing.
         implicit none
         class(tens_oprnd_t), intent(inout):: this   !inout: active tensor operand
         integer(INTD), intent(out), optional:: ierr !out: error code
         integer(INTD):: errc,pid,cid
         ...
         if(this%is_active(errc)) then
          if(errc.eq.0) then
           if(associated(this%cache_entry)) then
            pid=this%cache_entry%get_owner_id(errc)
            if(errc.eq.0) then
             cid=this%cache_entry%get_owner_id(errc,as_child=.TRUE.)
             if(errc.eq.0) then
              this%owner_id(1:2)=(/pid,cid/) !(parent,child)
             else
              errc=-4
             endif
            else
...
!tensor structure (its subtensor composition) is known.
         ...
         class(tens_oprnd_t), intent(inout):: this   !in: active tensor operand
         integer(INTD), intent(out), optional:: ierr !out: error code
         logical, intent(out), optional:: remote     !out: TRUE if the operand is remote (determined via the owner id as a parent)
         ...
         if(errc.ne.0) then !default values on error
...
!$OMP CRITICAL (IO)
         ...
         do j=1,nsp+1; write(devo,'(" ")',ADVANCE='NO'); enddo
         write(devo,'("Active = ",l1,"; Communication = ",i2,"; Metadata owner = (",i4,1x,i4,")")') actv,sts,this%owner_id(1:2)
!$OMP END CRITICAL (IO)
         if(associated(this%tensor)) then
...
! 5. Instruction operands (optional): {Owner_id,Read_count,Write_count,Tensor} for each tensor operand.
         ...
         integer(INTD), intent(out), optional:: ierr     !out: error code
         integer(INTD), intent(in), optional:: direction !in: direction of dispatch of the encoded instruction: {DSVP_INSTR_DIR_DOWN,DSVP_INSTR_DIR_SIDE,DSVP_INSTR_DIR_UP}
         integer(INTD):: errc,op_code,stat,err_code
         integer(INTL):: iid
         logical:: as_parent

!Pack the instruction attributes (id,op_code,status,error):
         as_parent=.TRUE.; if(present(direction)) as_parent=(direction.ne.DSVP_INSTR_DIR_DOWN)
         ...
         subroutine encode_instr_tens_create_destroy(jerr)
          ...
          if(jerr.eq.DSVP_SUCCESS) then
           ...
           class is(tens_oprnd_t)
            ...
            if(jerr.eq.0) then
             if(tensor%is_set()) then
              call pack_builtin(instr_packet,oprnd%get_owner_id(as_child=(.not.as_parent)),jerr)
...
!Configures this DSVU.
         ...
         type is(tavp_mng_decoder_conf_t)
          if(associated(conf%acceptor)) then
           ...
           this%decodes_from=conf%decodes_from
...
!$OMP END CRITICAL (IO)
            ...
            if(num_packets.gt.0) then
             if(uid.eq.0) then !uDecoder
!$OMP ATOMIC WRITE
              tavp%channel_in=channel
             elseif(uid.eq.6) then !cDecoder
              call tavp%dispatcher%update_issue_count(channel,int(-num_packets,INTL)) !decrement the number of active (issued) instructions
             endif
 !Decode new bytecode:
...
!$OMP END CRITICAL (IO)
  ...
  !Construct the instruction by decoding the extracted instruction:
              ...
              if(DEBUG.gt.0) then
               if(uid.eq.0) then !uDecoder only
!$OMP CRITICAL (IO)
...
!$OMP END CRITICAL (IO)
  ...
  !TAVP_INSTR_CTRL_DUMP_CACHE needs a special treatment:
               ...
               if(DEBUG.gt.0) then
!$OMP CRITICAL (IO)
                write(CONS_OUT,'("#DEBUG(TAVP-MNG:Decoder): Decoding paused for tensor cache dumping!")')
!$OMP END CRITICAL (IO)
                flush(CONS_OUT)
               endif
...
!$OMP CRITICAL (IO)
          write(CONS_OUT,'("#ERROR(TAVP-MNG)[",i6,"]: Decoder error ",i11," by thread ",i2)') impir,errc,thid
!$OMP END CRITICAL (IO)
          flush(CONS_OUT)
         endif
!Shutdown:
...
!$OMP END CRITICAL (IO)
         ...
         contains
          ...
          subroutine decode_instr_operands(jerr)
           ...
           integer(INTD):: jj,jn,jown,jread,jwrite,jpwn,jcwn
...
!$OMP END CRITICAL (IO)
             ...
             jpwn=tens_mng_entry%get_owner_id() !owner id as a parent from the tensor cache
             jcwn=tens_mng_entry%get_owner_id(as_child=.TRUE.) !owner id as a child from the tensor cache
...
!$OMP END CRITICAL (IO)
             ...
             if(updated.and.jown.ge.0) then !anonymous metadata updates (jown < 0) do not change the current owner
              if(tens_mng_entry%has_one_owner()) then !single-owner tensor cache entry (splitted tensor)
               call tens_mng_entry%set_owner_id(jown) !update the tensor owner id (as a parent and as a child) if the tensor info has been updated in the cache
              else
               if(this%decodes_from.eq.DSVP_INSTR_DIR_DOWN) then
                call tens_mng_entry%set_owner_id(jpwn,as_child=jown) !update only the owner id as a child
               else
                call tens_mng_entry%set_owner_id(jown,as_child=jcwn) !update only the owner id as a parent
               endif
              endif
...
!Starts and lives this DSVU, calls .shutdown() at the end.
         ...
         integer(INTD), intent(out), optional:: ierr     !out: error code
         integer(INTD):: errc,ier,thid,opcode,sts,iec,n,i,uid,channel
...
!Reserve a bytecode buffer:
         call this%bytecode%reserve_mem(ier,MAX_BYTECODE_SIZE,MAX_BYTECODE_INSTR); if(ier.ne.0.and.errc.eq.0) errc=-31
!Initialize queues and ports:
...
!Work loop:
 ...
 !Get a new batch of retired instructions (and control instructions) from Collector (port 0):
          ...
          ier=this%flush_port(0,num_moved=n); if(ier.ne.DSVP_SUCCESS.and.errc.eq.0) then; errc=-26; exit wloop; endif
...
!$OMP END CRITICAL (IO)
 ...
 !Encode the retired instructions into bytecode and send them to the upper level:
          ...
          ier=this%iqueue%get_status(); if(stopping.and.ier.ne.GFC_IT_EMPTY.and.errc.eq.0) then; errc=-24; exit wloop; endif
  ...
  !Extract an instruction:
           ...
           if(sts.ne.DS_INSTR_COMPLETED.and.errc.eq.0) then; errc=-20; exit wloop; endif !trap
...
!$OMP END CRITICAL (IO)
  ...
  !Encode a retired tensor instruction into bytecode:
           ...
           if(opcode.ge.TAVP_ISA_TENS_FIRST.and.opcode.le.TAVP_ISA_TENS_LAST) then
            if(opcode.eq.TAVP_INSTR_TENS_DESTROY) then
             call tens_instr%remove_persistency(ier); if(ier.ne.0.and.errc.eq.0) then; errc=-17; exit wloop; endif
            endif
            call tens_instr%extract_cache_entries(cache_entries,n,ier) !eviction candidates
            if(ier.ne.0.and.errc.eq.0) then; errc=-16; exit wloop; endif
            ...
            call this%encode(tens_instr,instr_packet,ier); if(ier.ne.0.and.errc.eq.0) then; errc=-14; exit wloop; endif
            call this%bytecode%seal_packet(ier); if(ier.ne.PACK_SUCCESS.and.errc.eq.0) then; errc=-13; exit wloop; endif
  ...
  !Delete the instruction from the main queue:
           ier=this%iqueue%delete(); if(ier.ne.GFC_SUCCESS.and.errc.eq.0) then; errc=-11; exit wloop; endif
  !Evict unneded remote tensor cache entries:
           if(n.gt.0) then
            do i=1,n
             tensor=>cache_entries(i)%cache_entry%get_tensor(ier)
             if(ier.eq.0) then
              evicted=this%arg_cache%evict(tensor,ier,decr_use=.TRUE.); if(ier.ne.0.and.errc.eq.0) errc=-10
             else
              if(errc.eq.0) errc=-9
             endif
  ...
  !Send the bytecode to the parent TAVP at the upper level:
           ...
           if(n.gt.0.and.(n.ge.MAX_RETIRE_INSTR.or.this%iqueue%get_status().eq.GFC_IT_EMPTY)) then
            call comm_hl%clean(ier); if(ier.ne.PACK_SUCCESS.and.errc.eq.0) then; errc=-8; exit wloop; endif
!$OMP ATOMIC READ
            channel=tavp%channel_in
            call this%bytecode%set_tag(channel,ier); if(ier.ne.0.and.errc.eq.0) then; errc=-7; exit wloop; endif
...
!Record the error:
         ...
         if(errc.ne.0.and.VERBOSE) then
!$OMP CRITICAL (IO)
          write(CONS_OUT,'("#ERROR(TAVP-MNG)[",i6,"]: Retirer error ",i11," by thread ",i2)') impir,errc,thid
!$OMP END CRITICAL (IO)
          flush(CONS_OUT)
         endif
!Shutdown:
...
!Encodes a tensor instruction into a plain bytecode.
         ...
         if(ds_instr%is_active(errc)) then
          if(errc.eq.DSVP_SUCCESS) then
           call ds_instr%encode(instr_packet,errc,DSVP_INSTR_DIR_UP); if(errc.ne.PACK_SUCCESS) errc=-3
          else
           errc=-2
          endif
         else
...
!Record the error:
         ...
         if(errc.ne.0.and.VERBOSE) then
!$OMP CRITICAL (IO)
          write(CONS_OUT,'("#ERROR(TAVP-MNG)[",i6,"]: Locator error ",i11," by thread ",i2)') impir,errc,thid
!$OMP END CRITICAL (IO)
          flush(CONS_OUT)
         endif
!Shutdown:
...
!Encodes a tensor instruction into a plain bytecode.
         ...
         if(ds_instr%is_active(errc)) then
          if(errc.eq.DSVP_SUCCESS) then
           call ds_instr%encode(instr_packet,errc,DSVP_INSTR_DIR_SIDE); if(errc.ne.0) errc=-3
          else
           errc=-2
          endif
         else
...
!Record the error:
         ...
         if(errc.ne.0.and.VERBOSE) then
!$OMP CRITICAL (IO)
          write(CONS_OUT,'("#ERROR(TAVP-MNG)[",i6,"]: Decomposer error ",i11," by thread ",i2)') impir,errc,thid
!$OMP END CRITICAL (IO)
          flush(CONS_OUT)
         endif
!Shutdown:
...
!$OMP END CRITICAL (IO)
                  ...
                  if(.not.(jerr.eq.0.and.associated(tens_entry))) then
                   deallocate(subtensor); jerr=-22; exit cloop
                  endif
                  tens_entry_mng=>NULL()
                  ...
                  if(.not.stored) then !subtensor is already in the tensor cache (in case a tensor is composed of itself)
                   ...
                   subtensor=>tens_entry%get_tensor(jerr); if(jerr.ne.0) then; jerr=-21; exit cloop; endif
                   call tens_entry_mng%set_owner_id(tens_entry_mng%get_owner_id(),jerr,as_child=-1) !reset owner id as a child to undefined (will be set by Dispatcher)
                   if(jerr.ne.0) then; jerr=-20; exit cloop; endif
                  endif
...
!$OMP END CRITICAL (IO)
             ...
             flush(jo)
             call tavp%resume_decode() !decode was paused for tensor cache dumping
             if(DEBUG.gt.0) then
!$OMP CRITICAL (IO)
              write(CONS_OUT,'("#DEBUG(TAVP-MNG:Dispatcher): Decoding resumed after tensor cache dumping!")')
!$OMP END CRITICAL (IO)
              flush(CONS_OUT)
             endif
            endif
...
!Record the error:
         ...
         if(errc.ne.0.and.VERBOSE) then
!$OMP CRITICAL (IO)
          write(CONS_OUT,'("#ERROR(TAVP-MNG)[",i6,"]: Dispatcher error ",i11," by thread ",i2)') impir,errc,thid
!$OMP END CRITICAL (IO)
          flush(CONS_OUT)
         endif
!Shutdown:
...
!Encodes a tensor instruction into a plain bytecode.
         ...
         if(ds_instr%is_active(errc)) then
          if(errc.eq.DSVP_SUCCESS) then
           call ds_instr%encode(instr_packet,errc,DSVP_INSTR_DIR_DOWN); if(errc.ne.PACK_SUCCESS) errc=-3
          else
           errc=-2
          endif
         else
            ...
            if(errc.eq.0) then
             if(n.gt.0) then
              ...
              do i=lbound(out_oprs,1),ubound(out_oprs,1)
               oprnd=>tens_instr%get_operand(out_oprs(i),errc); if(errc.ne.DSVP_SUCCESS) then; errc=-14; exit; endif
               select type(oprnd)
               class is(tens_oprnd_t)
                pid=oprnd%get_owner_id(errc)
                if(errc.eq.0) then
                 if(pid.lt.0) then !no owner at all
                  call oprnd%set_owner_id(home,errc,update_cache=.TRUE.); if(errc.ne.0) then; errc=-13; exit; endif
                 else !owner id as a parent is set
                  if(oprnd%get_owner_id(errc,as_child=.TRUE.).lt.0) then !owner id as a child is absent (unsplit tensor)
                   if(errc.eq.0) then
                    call oprnd%set_owner_id(pid,errc,update_cache=.TRUE.,as_child=home); if(errc.ne.0) then; errc=-12; exit; endif
                   else
                    errc=-11; exit
                   endif
                  else
...
!Issues the encoded bytecode to a specific dispatch channel.
         ...
         if(channel.ge.lbound(this%dispatch_rank,1).and.channel.le.ubound(this%dispatch_rank,1)) then
          if(this%comm_hl(channel)%is_clean(errc)) then
           if(errc.eq.0) then
            call this%bytecode(channel)%set_tag(channel,errc)
            if(errc.eq.0) then
...
!$OMP ATOMIC UPDATE
          this%issue_count(channel)=this%issue_count(channel)+delta
         else
          errc=-1
         endif
         if(present(ierr)) ierr=errc
...
!Record the error:
         ...
         if(errc.ne.0.and.VERBOSE) then
!$OMP CRITICAL (IO)
          write(CONS_OUT,'("#ERROR(TAVP-MNG)[",i6,"]: Replicator error ",i11," by thread ",i2)') impir,errc,thid
!$OMP END CRITICAL (IO)
          flush(CONS_OUT)
         endif
!Shutdown:
...
!Record the error:
         ...
         if(errc.ne.0.and.VERBOSE) then
!$OMP CRITICAL (IO)
          write(CONS_OUT,'("#ERROR(TAVP-MNG)[",i6,"]: Collector error ",i11," by thread ",i2)') impir,errc,thid
!$OMP END CRITICAL (IO)
          flush(CONS_OUT)
         endif
!Shutdown:
 ...
 !Configure static DSVU:
  !Up-decoder:
             ...
             decoder_conf=tavp_mng_decoder_conf_t(source_comm=conf%source_comm,source_rank=conf%source_rank,&
              &msg_tag=TAVP_DISPATCH_TAG,acceptor=decode_acceptor,acceptor_port_id=0,decodes_from=DSVP_INSTR_DIR_UP)
  ...
  !Locating-decoder:
                ...
                decoder_conf=tavp_mng_decoder_conf_t(source_comm=conf%ring_comm,source_rank=conf%ring_recv_rank,&
                 &msg_tag=TAVP_LOCATE_TAG,acceptor=decode_acceptor,acceptor_port_id=1,decodes_from=DSVP_INSTR_DIR_SIDE)
  ...
  !Replicating-decoder:
                    ...
                    decoder_conf=tavp_mng_decoder_conf_t(source_comm=role_comm,source_rank=MPI_ANY_SOURCE,&
                     &msg_tag=TAVP_REPLICA_TAG,acceptor=decode_acceptor,acceptor_port_id=1,decodes_from=DSVP_INSTR_DIR_SIDE)
  ...
  !Collecting-decoder:
                     ...
                     decoder_conf=tavp_mng_decoder_conf_t(source_comm=conf%collect_comm,source_rank=MPI_ANY_SOURCE,&
                      &msg_tag=TAVP_COLLECT_TAG,acceptor=decode_acceptor,acceptor_port_id=1,decodes_from=DSVP_INSTR_DIR_DOWN)
...
!$OMP END CRITICAL (TAVP_MNG_INSTR)
         ...
         if(VERBOSE.and.errc.ne.0) then
!$OMP CRITICAL (IO)
          write(CONS_OUT,'("#ERROR(TAVP-MNG.map_instr): Error ",i11)') errc !debug
!$OMP END CRITICAL (IO)
          flush(CONS_OUT)
         endif
         if(present(ierr)) ierr=errc
