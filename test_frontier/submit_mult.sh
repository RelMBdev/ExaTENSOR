...
export QF_GPUS_PER_PROCESS=1       #number of discrete NVIDIA GPUs per MPI process (optional)
...
# OpenMP environment
export OMP_THREAD_LIMIT=14                  #max total number of OpenMP threads per process
export OMP_NUM_THREADS=${QF_NUM_THREADS}    #initial number of OpenMP threads per MPI process (=QF_NUM_THREADS)
export OMP_DYNAMIC=TRUE                    #no OpenMP dynamic threading
...
export MPICH_ASYNC_PROGRESS=1                 #requires additional processor 
...
#export MPICH_MPIIO_OFI_STARTUP_CONNECT=1
...
${temp_dir}
submit command:
time srun -N${SLURM_NNODES} -n${QF_NUM_PROCS} -c${QF_CORES_PER_PROCESS} --gpus-per-task=${QF_GPUS_PER_PROCESS} --gpu-bind=closest  ./Qforce.x >& qforce.output
EOF
