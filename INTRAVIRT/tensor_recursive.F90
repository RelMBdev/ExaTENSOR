...
!TYPES:
 ...
 !Tensor header (signature+shape):
        type, public:: tens_header_t
         ...
         contains
          ...
          procedure, private:: TensHeaderPrintHeadFile              !prints only the signature and shape in one line in file
 ...
 !Tensor body:
        type, public:: tens_body_t
         ...
         integer(INTD), private:: data_type=NO_TYPE           !tensor body data type: {R4,R8,C4,C8}
         contains
          ...
          procedure, public:: get_data_type=>TensBodyGetDataType    !returns the tensor body numeric data type
 ...
 !Recursive tensor:
        type, public:: tens_rcrsv_t
         ...
         contains
          ...
          procedure, private:: TensRcrsvPrintHeadFile                !prints only the signature+shape in one line in file
 ...
 !Tensor transformation (includes initialization and prefactor scaling as specific cases):
        type, extends(tens_operation_t), public:: tens_transformation_t
         ...
         contains
          ...
          procedure, private:: import_replace=>TensTransformationImportReplace !creates a new tensor transformation by replacing the tensor argument in an existing tensor transformation
          procedure, private:: TensTransformationSplitFunc                  !splits the tensor transformation into a list of subtensor transformations based on the externally provided tensor splitting function
          procedure, private:: TensTransformationSplitIntern                !splits the tensor transformation into a list of subtensor transformations based on the internal list of constituent subtensors
          generic, public:: split=>TensTransformationSplitFunc,TensTransformationSplitIntern
          procedure, public:: print_it=>TensTransformationPrintIt           !prints the tensor transformation
...
!VISIBILITY:
 ...
 !tens_header_t:
        ...
        private TensHeaderPrintHeadFile
        private TensHeaderPrintHeadString
 ...
 !tens_body_t:
        ...
        private TensBodySetDataType
        private TensBodyGetDataType
 ...
 !tens_rcrsv_t:
        ...
        private TensRcrsvPrintHeadFile
        private TensRcrsvPrintHeadString
 ...
 !tens_transformation_t:
        ...
        private TensTransformationImportReplace
        private TensTransformationSplitFunc
        private TensTransformationSplitIntern
        private TensTransformationPrintIt
...
!-------------------------------------------------------------------
        subroutine TensHeaderPrintHeadFile(this,ierr,dev_id,nspaces)
...
! tensor_name(HSpaceId:SubspaceId,...)[DimExtent,...]
         ...
         if(present(ierr)) ierr=errc
         return
        end subroutine TensHeaderPrintHeadFile
!------------------------------------------------------------------
        subroutine TensHeaderPrintHeadString(this,str,str_len,ierr)
!Prints only the signature and shape in one line. Format:
! tensor_name(HSpaceId:SubspaceId,...)[DimExtent,...]
         implicit none
         class(tens_header_t), intent(in):: this     !in: tensor header
         character(*), intent(inout):: str           !out: output string
         integer(INTD), intent(out):: str_len        !out: length of <str>
         integer(INTD), intent(out), optional:: ierr !out: error code
         integer(INTD):: errc,i,j,l,k,n
         integer(INTL):: il
         ...
         if(present(ierr)) ierr=errc
         return
        end subroutine TensHeaderPrintHeadString
!-------------------------------------------------------
...
!Constructs the "Fortran-dimension-led" tensor body storage layout.
...
![tens_body_t]==========================================
        subroutine TensBodyCtorBase(this,ierr,data_type)
!Default ctor.
         implicit none
         class(tens_body_t), intent(out):: this          !out: tensor body
         integer(INTD), intent(out), optional:: ierr     !out: error code
         integer(INTD), intent(in), optional:: data_type !in: tensor body numeric data type: {R4,R8,C4,C8}

         if(present(data_type)) this%data_type=data_type
...
!if it consists of at least one subtensor.
         ...
         integer(INTD), intent(out), optional:: ierr !out: error code
         logical, intent(out), optional:: layed      !out: TRUE if the tensor storage layout has been set
...
!----------------------------------------------------------
        subroutine TensBodySetDataType(this,data_type,ierr)
!Resets the tensor body numeric data type. If the tensor layout
!has already been allocated, the new data type cannot differ from it.
         implicit none
         class(tens_body_t), intent(inout):: this    !inout: tensor body
         integer(INTD), intent(in):: data_type       !in: data type
         integer(INTD), intent(out), optional:: ierr !out: error code
         integer(INTD):: errc,dts

         errc=TEREC_SUCCESS
         if(tens_valid_data_kind(data_type,dts).eq.YEP) then
          if(allocated(this%layout)) then !check consistency
           if(this%layout%get_data_type(errc).ne.data_type) then
            if(errc.eq.TEREC_SUCCESS) errc=TEREC_INVALID_REQUEST
           endif
          endif
          if(errc.eq.TEREC_SUCCESS) this%data_type=data_type
         else
          errc=TEREC_INVALID_ARGS
         endif
         if(present(ierr)) ierr=errc
         return
        end subroutine TensBodySetDataType
!----------------------------------------------------------------
        function TensBodyGetDataType(this,ierr) result(data_type)
!Returns the tensor body numeric data type.
         implicit none
         integer(INTD):: data_type                   !out: data type
         class(tens_body_t), intent(in):: this       !in: tensor body
         integer(INTD), intent(out), optional:: ierr !out: error code
         ...
         if(present(ierr)) ierr=TEREC_SUCCESS
         return
        end function TensBodyGetDataType
!-----------------------------------------------------------
...
!from the tensor body will be used (if set, otherwise error).
         ...
         if(this%is_set(errc,layd,locd)) then
          if(errc.eq.TEREC_SUCCESS.and.(.not.layd)) then
           dt=this%data_type; if(present(data_type)) dt=data_type
           if((this%data_type.eq.NO_TYPE.and.dt.ne.NO_TYPE).or.(this%data_type.ne.NO_TYPE.and.this%data_type.eq.dt)) then
            this%data_type=dt
...
!---------------------------------------------------------------------
        subroutine TensRcrsvSetLayout(this,layout_kind,ierr,data_type)
...
!-----------------------------------------------------------
        subroutine TensRcrsvSetDataType(this,data_type,ierr)
!Resets the tensor body data type, unless the tensor has already been laid-out with some data type.
         implicit none
         class(tens_rcrsv_t), intent(inout):: this   !inout: tensor
         integer(INTD), intent(in):: data_type       !in: data type
         integer(INTD), intent(out), optional:: ierr !out: error code
         ...
         call this%body%set_data_type(data_type,errc)
         if(present(ierr)) ierr=errc
         return
        end subroutine TensRcrsvSetDataType
!-----------------------------------------------------------------
...
!Returns the data type of the tensor elements.
         ...
         data_type=this%body%get_data_type(errc)
         if(present(ierr)) ierr=errc
         return
...
!------------------------------------------------------------------
        subroutine TensRcrsvPrintHeadFile(this,ierr,dev_id,nspaces)
!Prints only the tensor signature+shape in one line.
         ...
         if(present(ierr)) ierr=errc
         return
        end subroutine TensRcrsvPrintHeadFile
!-----------------------------------------------------------------
        subroutine TensRcrsvPrintHeadString(this,str,str_len,ierr)
!Prints only the tensor signature+shape in one line.
         implicit none
         class(tens_rcrsv_t), intent(in):: this      !in: tensor
         character(*), intent(inout):: str           !out: output string
         ...
         call this%header%print_head(str,str_len,errc)
         if(present(ierr)) ierr=errc
         return
        end subroutine TensRcrsvPrintHeadString
!------------------------------------------------------
...
!-----------------------------------------------------------------------------------
        subroutine TensTransformationImportReplace(this,tens_trans,clone,ierr,dtens)
...
!---------------------------------------------------------------------------------------
        subroutine TensTransformationSplitFunc(this,tens_split_f,subops,ierr,num_subops)
!Splits a defined tensor transformation into a list of subtransformations. The splitting
!is driven by the tensor argument decomposition into unique subtensors. More precisely,
!the tensor argument is represented as a direct sum of its unique child subtensors.
!The subtensors are generated by the externally provided function <tens_split_f()>
!which is expected either to fill in a vector of subtensors by storing them by value
!or to generate the subtensors in a persistent storage and fill in a vector of references
!to those subtensors.
         implicit none
         ...
         if(this%is_set(errc)) then
          if(errc.eq.TEREC_SUCCESS) then
 !Get the parental tensor:
           tensor=>this%get_argument(0,errc)
           if(errc.eq.TEREC_SUCCESS) then
 !Generate a vector of subtensors (stored by value):
 ...
 !Construct a list of subtransformations:
             ...
             if(errc.eq.TEREC_SUCCESS) errc=TEREC_UNABLE_COMPLETE
            endif
           endif
           tensor=>NULL()
          endif
         ...
         if(present(ierr)) ierr=errc
         return
        end subroutine TensTransformationSplitFunc
!----------------------------------------------------------------------------
        subroutine TensTransformationSplitIntern(this,subops,ierr,num_subops)
!Same as TensTransformationSplitFunc but the tensor composition is generated by the internal tensor structure.
         implicit none
         class(tens_transformation_t), intent(in):: this   !in: parental tensor transformation
         ...
         n=0
         call this%TensTransformationSplitFunc(tens_rcrsv_dim_split_default,subops,errc,n)
         if(present(num_subops)) num_subops=n
         if(present(ierr)) ierr=errc
         return
        end subroutine TensTransformationSplitIntern
!---------------------------------------------------------------------
        subroutine TensTransformationPrintIt(this,ierr,dev_id,nspaces)
!Prints the tensor transformation info.
         implicit none
         class(tens_transformation_t), intent(in):: this !in: tensor transformation
         integer(INTD), intent(out), optional:: ierr     !out: error code
         integer(INTD), intent(in), optional:: dev_id    !in: output device (defaults to screen)
...
!$OMP CRITICAL (IO)
          ...
          if(allocated(this%definer_name)) then
           ...
           call printl(devo,'External method: None')
          endif
          do i=1,nsp+1; write(devo,'(" ")',ADVANCE='NO'); enddo
...
!$OMP END CRITICAL (IO)
         ...
         if(present(ierr)) ierr=errc
         return
        end subroutine TensTransformationPrintIt
!------------------------------------------------
